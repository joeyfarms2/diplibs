...
/* Added by: Oui
/****************************************************************/
function upload_image_gallery($field_image_name,$upload_path,$new_file_name="",$max_file_size=150000,$width=9999,$height=9999,$qulity=99,$ratio=0,$fid=""){
	//echo print_r($_FILES[$field_image_name])."<hr>";
	require_once("./include/image_toolbox.class.php");
	if( !is_blank($field_image_name) && !is_blank($upload_path) ){
		...
		if (is_var_array($_FILES[$field_image_name]['tmp_name']) && $fid >= 0) {
			$_tmp_name = $_FILES[$field_image_name]['tmp_name'][$fid];
			$_name = $_FILES[$field_image_name]['name'][$fid];
			$_size = $_FILES[$field_image_name]['size'][$fid];
		}
		else {
			$_tmp_name = $_FILES[$field_image_name]['tmp_name'];
			$_name = $_FILES[$field_image_name]['name'];
			$_size = $_FILES[$field_image_name]['size'];
		}

		$file_info = getimagesize($_tmp_name);
		$file_width = $file_info[0];
		$file_height = $file_info[1];
		...
		$file_type = substr(strrchr($_name, "."), 1);
		if( strtolower($file_type) != "jpg" && strtolower($file_type) != "jpeg" && strtolower($file_type) != "gif" && strtolower($file_type) != "png" ){
			$data["error_msg"] = "file_type ไม่รองรับ (_name = ".$_name.") (file_type = ".$file_type.") รูปจะต้องเป็น JPG, JPEG, GIF หรือ PNG เท่านั้น.";
			return $data;
		}
		...
		$ITB_thumb = new Image_Toolbox($_tmp_name);
