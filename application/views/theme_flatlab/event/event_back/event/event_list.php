...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('eventBackDataSearchSession'); 
}
...
$search_event_category = get_array_value($data_search,"search_event_category","");
$master_event_category = @$master_event_category;

// print_r($data_search);
?>
<form id="frm_event" name="frm_event" method="POST" action="" class="form-horizontal tasi-form">
...
<input type="hidden" id="search_order_by" name="search_order_by" value="<?=$search_order_by?>" />
	<div class="row">
		<div class="col-xs-12 m-bot15">
			<a href="<?=site_url('admin/event/add')?>" class="btn btn-primary">
			...
			<section class="panel">
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group">
						...
						<div class="col-md-12 col-lg-8">
							<label class="checkbox-inline">
								<input type="checkbox" id="search_event_category_all" name="search_event_category_all" onclick="changeCheckAll('search_event_category_all','search_event_category[]',false,false);showSearchResult();" />All
							</label>
							<?php 
								if(is_var_array($master_event_category)){ 
									foreach($master_event_category as $m_item){
							?>
									<label class="checkbox-inline">
										<input type="checkbox" id="search_event_category_<?=get_array_value($m_item,"aid","")?>" name="search_event_category[]" onclick="changeCheckItem('search_event_category_all','search_event_category[]',false,true);showSearchResult();" value="<?=get_array_value($m_item,"aid","")?>" <?php if(is_in_array(get_array_value($m_item,"aid",""),$search_event_category)) echo 'checked="checked"';?>  /><?=get_array_value($m_item,"name","")?>
									</label>
								<?php } //foreach ?>
							<?php } //if ?>
						</div>
					...
					<div class="form-group">
						<div class="col-lg-offset-2 col-lg-8">
							...
							<a class="btn btn-default" onclick="ajaxClearSearchResult('eventBackDataSearchSession')" />Clear</a>
						</div>
					</div>
					<!-- End : Button -->
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		changeCheckItem('search_event_category_all','search_event_category[]',false,true);
