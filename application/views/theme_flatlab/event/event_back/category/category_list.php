...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('eventCategoryBackDataSearchSession'); 
}
...
$search_event_main = get_array_value($data_search,"search_event_main","");
...
<input type="hidden" id="search_order_by" name="search_order_by" value="<?=$search_order_by?>" />
	<div class="row">
		<div class="col-xs-12 m-bot15">
			<a href="<?=site_url('admin/event-category/add')?>" class="btn btn-primary">
		...
		<div class="col-xs-12">
			<section class="panel">
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group <?=(is_root_admin_or_higher() ? '' : 'hidden')?>">
						<label class="col-md-12 col-lg-2 control-label">Event main</label>
						<div class="col-md-12 col-lg-8">
							<label class="checkbox-inline">
								<input type="checkbox" id="search_event_main_all" name="search_event_main_all" onclick="changeCheckAll('search_event_main_all','search_event_main[]',false,false);showSearchResult();" />All
							</label>
							<?php 
								if(is_var_array($master_event_main)){ 
									foreach($master_event_main as $m_item){
							?>
									<label class="checkbox-inline">
										<input type="checkbox" id="search_event_main_<?=get_array_value($m_item,"aid","")?>" name="search_event_main[]" onclick="changeCheckItem('search_event_main_all','search_event_main[]',false,true);showSearchResult();" value="<?=get_array_value($m_item,"aid","")?>" <?php if(is_in_array(get_array_value($m_item,"aid",""),$search_event_main)) echo 'checked="checked"';?>  /><?=get_array_value($m_item,"name","")?>
									</label>
								<?php } //foreach ?>
							<?php } //if ?>
						</div>
					...
					<div class="form-group">
						<div class="col-lg-offset-2 col-lg-8">
							...
							<a class="btn btn-default" onclick="ajaxClearSearchResult('eventCategoryBackDataSearchSession')" />Clear</a>
						</div>
					</div>
					<!-- End : Button -->
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		changeCheckItem('search_event_main_all','search_event_main[]',false,true);
