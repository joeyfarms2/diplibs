...
if($init_adv_search != "clear"){
	...
	$data_search = $dataSearchSession->userdata('questionaireBackDataSearchSession'); 
}
...
$search_questionaire_category = get_array_value($data_search,"search_questionaire_category","");
$master_questionaire_category = @$master_questionaire_category;

// print_r($data_search);
?>
<form id="frm_questionaire" name="frm_questionaire" method="POST" action="" class="form-horizontal tasi-form">
...
<input type="hidden" id="search_order_by" name="search_order_by" value="<?=$search_order_by?>" />
	<div class="row">
		<div class="col-xs-12 m-bot15">
			<a href="<?=site_url('admin/questionaire/add')?>" class="btn btn-primary">
			...
			<section class="panel">
				...
				<div id="adv-area" class="panel-body" style="display: none;">
					...
					<div class="form-group">
						...
						<div class="col-md-12 col-lg-8">
							<label class="checkbox-inline">
								<input type="checkbox" id="search_questionaire_category_all" name="search_questionaire_category_all" onclick="changeCheckAll('search_questionaire_category_all','search_questionaire_category[]',false,false);showSearchResult();" />All
							</label>
							<?php 
								if(is_var_array($master_questionaire_category)){ 
									foreach($master_questionaire_category as $m_item){
							?>
									<label class="checkbox-inline">
										<input type="checkbox" id="search_questionaire_category_<?=get_array_value($m_item,"aid","")?>" name="search_questionaire_category[]" onclick="changeCheckItem('search_questionaire_category_all','search_questionaire_category[]',false,true);showSearchResult();" value="<?=get_array_value($m_item,"aid","")?>" <?php if(is_in_array(get_array_value($m_item,"aid",""),$search_questionaire_category)) echo 'checked="checked"';?>  /><?=get_array_value($m_item,"name","")?>
									</label>
								<?php } //foreach ?>
							<?php } //if ?>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-md-12 col-lg-8">
							...
							<label class="checkbox-inline">
								<input type="checkbox" id="search_status_0" name="search_status[]"  onclick="changeCheckItem('search_status_all','search_status[]',false,true);showSearchResult();" value="0" <?php if(is_in_array("0",$search_status)) echo "checked"; ?> />Draft
							</label>
							<label class="checkbox-inline">
								<input type="checkbox" id="search_status_1" name="search_status[]"  onclick="changeCheckItem('search_status_all','search_status[]',false,true);showSearchResult();" value="1" <?php if(is_in_array("1",$search_status)) echo "checked"; ?> />Published
							</label>
						</div>
					</div>
					...
					<div class="form-group">
						<div class="col-lg-offset-2 col-lg-8">
							...
							<a class="btn btn-default" onclick="ajaxClearSearchResult('questionaireBackDataSearchSession')" />Clear</a>
						</div>
					</div>
					<!-- End : Button -->
				</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		changeCheckItem('search_questionaire_category_all','search_questionaire_category[]',false,true);
