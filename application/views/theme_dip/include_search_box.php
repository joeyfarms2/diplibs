...
<form id="frm_search" name="frm_search" class="form-inline frm-right" role="form" method="get">
    <div class="">
        ...
        <select class="form-control opt-search hide" id="search_in_product_main" name="search_in_product_main">
            ...
            <?php if (is_var_array($master_product_main)) { ?>
                <?php foreach ($master_product_main as $item) { ?>
                    <option
                        value="<?= get_array_value($item, "aid", "N/A"); ?>"><?= get_array_value($item, "name", "N/A"); ?></option>
                <?php } ?>
            <?php } ?>
        </select>
        ...
        <input type="text" class="form-control search-txt-box" placeholder="Keyword" value="" id="keyword"
               name="keyword" onkeypress="isEnterGoTo(event,'search_advance(1)')"/>
    </div>
</form>
<!--					</section>-->
<!--				</div>-->
