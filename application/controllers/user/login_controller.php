...
class Login_controller extends User_init_controller {
	...
	function login()
	{
		...
		if($user_aid != ""){
			...
			if(CONST_LDAP_LOGIN > 0 && !is_blank($user_cookie) && !is_blank($pass_cookie)){
	...
	function check($user_cid,$user_pass,$owner_alias,$remember,$hash)
	{
		...
		if ((is_var_array($result)) || (!is_var_array($result))) {
				
				// Authen with LDAP Server
				if (CONST_LDAP_AUTHEN == TRUE && ((is_var_array($result) && $result['password'] == "") || (!is_var_array($result)))) {
					...
					*/
				...
				}elseif( CONST_LDAP_AUTHEN_SOAP == TRUE && ((is_var_array($result) && $result['password'] == "") || (!is_var_array($result)))) { 
						
						$result = $this->chack_soap_status($user_cid,$user_pass);
				...
				}elseif( CONST_LDAP_AUTHEN_JSON == TRUE && ((is_var_array($result) && $result['password'] == "") || (!is_var_array($result))) ){

						

						$result = $this->chack_json_status($user_cid,$user_pass);
					...
					if (CONST_LDAP_LOGIN > 0 && $result['password'] !== "") {
							$user_pass = $this->user->encryptPassword($user_pass);
					}
				// }
	...
	{
		// $this->data["page_title"] = $this->lang->line('user_login_header');
		$this->data["page_title"] = '<span>Login</span>';
		switch($type)
		{
			case md5('blank') : 
	...
	function chack_soap_status($user_cid="",$user_pass="")
	{
								...
								if(!is_var_array($result_user)){
									...
									if(preg_match("/^[0-9]+$/", $user_cid)){
											...
											if ($ldap_new_user_aid > 0) {
												$this->log_status('Auto insert new username ['.$user_cid.'] into database.', 'Access', 'User aid = '.$ldap_new_user_aid);
												$this->user->set_where(array("aid"=>$ldap_new_user_aid));
												$result = $this->user->load_record(true);

											}else{
												$this->log_status('Failed to auto insert new username ['.$user_cid.'] into our database.', 'Access', '');
		...
		function chack_json_status($user_cid="",$user_pass=""){

			//header('Content-Type: text/html; charset=utf-8');

			$client = CONST_LDAP_AUTHEN_JSON_URL."?UserName=".$user_cid."&Password=".$user_pass;

			$jsondata = file_get_contents($client);
			...
			foreach ($obj as $key => $value) {
				...
				if(is_var_array($value)){
					...
					$chack = get_array_value($value,"status","");

					$user_cid = get_array_value($value,"memberID","");

					$MemberName = get_array_value($value,"MemberName","");

					$email = get_array_value($value,"email","");

					$department = get_array_value($value,"DepName","");

					$user_section_aid = get_array_value($value,"MemberType","");

					$Name = explode(" ", $MemberName);

					$first_name_th = $Name[0];

					$last_name_th = $Name[1];

					

					if(strpos( $first_name_th, "นางสาว")) {
					...
					if($chack == "yes"){
							...
							// print_r($result_user);
								...
								if(!is_var_array($result_user)){
									...
									$ldap_data_user["cid"] = $user_cid;
									
									$ldap_data_user["username"] = $user_cid;
									
									$ldap_data_user["first_name_th"] = $first_name_th; // $obj_user->firstname; // $username;
											
									$ldap_data_user["last_name_th"] =  $last_name_th; // $obj_user->surname; // $username;
									
									$ldap_data_user["user_section_aid"] = $user_section_aid;
									
									$ldap_data_user["user_role_aid"] = "5";
									
									$ldap_data_user["email"] = $email;
									
									$ldap_data_user["registration_date"] = get_db_now();

									$ldap_data_user["gender"] = $gender;

									$ldap_data_user["note_1"] = $department;
									...
									$user_aid = get_array_value($result_user,"aid","");

									$data = array();

									$data["username"] = $user_cid;
											
									$data_where = array();
										
									$data_where["first_name_th"] = $first_name_th; // $obj_user->firstname; // $username;
											
									$data_where["last_name_th"] =  $last_name_th; // $obj_user->surname; // $username;
									
									$data_where["user_section_aid"] = $user_section_aid;
									
									$data_where["email"] = $email;

									$data_where["note_1"] = $department;

									$this->load->model($this->model,'user');

									$this->user->set_where($data);

									$user_up = $this->user->update_record($data_where);
