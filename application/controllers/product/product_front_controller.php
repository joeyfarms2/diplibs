...
class Product_front_controller extends Product_init_controller {
	...
	if($product_type_aid == 3){
			if(get_array_value($item_result,"product_main_aid","0") != 9){
				$this->data["page_title"] = get_array_value($product_type_detail,"name","N/A").'Detail';
				$this->data["view_the_content"] = $this->default_theme_front . '/' . folderName . '/vdo_detail';
			}else{
				$this->data["page_title"] = get_array_value($item_result,"product_main_name","N/A").'Detail';
				$this->data["view_the_content"] = $this->default_theme_front . '/' . folderName . '/gallery_detail';
			}
		}else{
			$this->data["page_title"] = get_array_value($product_type_detail,"name","N/A").'Detail';
			$this->data["view_the_content"] = $this->default_theme_front . '/' . folderName . '/product_detail';
	...
	if($product_type_aid == '3'){
				...
				$parent_aid = get_array_value($item_result,"aid","");
				$this->load->model(get_array_value($model,"product_gallerys_model",""),"product_gallerys");
				//$this->load->model($this->vdo_gallerys_model,"vdo_gallerys");
				$tmp = array();
				$tmp["parent_aid"] = $parent_aid;
				$tmp["status"] = "1";
				$this->product_gallerys->set_where($tmp);
				$this->product_gallerys->set_order_by("weight ASC, created_date ASC");
				$product_gallery_list = $this->product_gallerys->load_records(true);
				//echo "<br>sql : ".$this->db->last_query();
				$item_result["product_gallery_list"] = $product_gallery_list;
			}
