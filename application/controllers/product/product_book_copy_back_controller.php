...
class Product_book_copy_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		$this->main_model = 'Book_copy_model';
	...
	function check_exits_book($parent_aid="",$return_json=false){
	...
	function show($product_main_url="", $parent_aid="", $msg=""){
		...
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/book_copy_list';
	...
	function form($product_main_url="", $parent_aid=""){
		define("thisAdminTabMenu",'product_book_'.$product_main_url);
		$this->data["title"] = DEFAULT_TITLE;
		$this->data["view_the_content"] = $this->default_theme_admin . '/' . folderName . '/book_copy_form';
	...
	function edit($product_main_url="", $parent_aid="", $aid=""){
		...
		if(CONST_HAS_TRANSACTION == "1"){
			...
			$this->book_ref_user_section->set_where(array("copy_aid"=>$aid));
			$this->data["product_ref_user_section_result"] = $this->book_ref_user_section->load_records_array(false, "user_section_aid", "");
		}
			$this->form($product_main_url, $parent_aid);
		}else{
			$this->data["message"] = set_message_error('Cannot find the specific book copy.');
			$this->data["js_code"] = "";
	...
	function save(){
		...
		if($command == "_update"){
			...
			if(!is_var_array($itemResult)){
				redirect('admin/product-'.$product_main_url.'/book/edit/'.$parent_aid.'/copy/status/'.md5('no-book-copy'));
				return "";
			}
		...
		if(!is_in_array($type, $type_master)){
			redirect('admin/product-'.$product_main_url.'/book/status/'.md5('copy-type-incorrect'));
			return "";
		}
		if(!is_in_array($possession, $possession_master)){
			redirect('admin/product-'.$product_main_url.'/book/status/'.md5('copy-possession-incorrect'));
			return "";
		}
		...
		if($type == '1'){ // Digital
			...
			if($is_license == '1'){
				if($possession == '1'){
					redirect('admin/product-'.$product_main_url.'/book/status/'.md5('both-buyout-license'));
					return "";
				}
			...
			$digital_point = "";
			if($is_license == '1'){
				redirect('admin/product-'.$product_main_url.'/book/status/'.md5('both-paper-license'));
				return "";
			}else{
				$is_license = "";
				$ebook_concurrence = "";
		...
		if(is_blank($barcode)){
			...
			$data["barcode"] = $barcode;
			if($this->isBookBarcodeExits($barcode, $aid)){
		...
		if(is_blank($upload_path)){
			$upload_path = "uploads/".CONST_PROJECT_CODE."/book_copy/".get_datetime_pattern("Y",$today_date,"").'/'.get_datetime_pattern("m",$today_date,"").'/'.$barcode.'_'.$cid.'/';
		}
		...
		$upload_inc_path = $upload_base_path.'doc';
		create_directories($upload_inc_path);
		...
		*/
		...
		//echo $use_digital_gen;

		if( !is_blank(get_array_value($_FILES,"file_upload_doc","")) && !is_blank(get_array_value($_FILES["file_upload_doc"],"name","")) ){
			//Start upload file
			$upload_path = $upload_base_path.'doc';
			$file_name = $_FILES["file_upload_doc"]["name"];
			$file_type = substr(strrchr($file_name, "."), 0);
			
			$new_file_name = $cid.$file_type;
			$old_file = "./".$upload_path."/".$new_file_name;

			$result_file_upload = upload_file($this,"file_upload_doc",$upload_path,$new_file_name,CONST_ALLOW_FILE_TYPE_DEFAULT,CONST_ALLOW_FILE_SIZE_DEFAULT,'','');
		...
		if($command == "_insert"){
			...
			if($aid){
				...
				$this->log_status('Backend : Insert book copy', '[barcode='.$barcode.'] of ['.$title.'] just added into database.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Book copy [barcode = $barcode] has been added", $data, "copy-add");
				...
				redirect('admin/product-'.$product_main_url.'/book/edit/'.$parent_aid.'/copy/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert book copy', 'Command insert_record() fail. Can not insert [barcode='.$barcode.'] of ['.$title.']', $data);
			...
			if($rs){
				...
				$this->log_status('Admin : Update book copy',  '[barcode='.$barcode.'] of ['.$title.'] has been updated.', $data);
				$this->log_product_save($product_type_aid, $parent_aid, $title, "U", "Book copy [barcode = $barcode] has been updated", $data, "copy-update");
				...
				if($save_option){
					redirect('admin/product-'.$product_main_url.'/book/edit/'.$parent_aid.'/copy/add');
				}else{
					redirect('admin/product-'.$product_main_url.'/book/edit/'.$parent_aid.'/copy/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update book copy', 'Command update_record() fail. Can not update [barcode='.$barcode.'] of ['.$title.']', $data);
				$this->data["message"] = set_message_error('Sorry, The system can not insert data right now.<BR>Please try again later or contact administrator to solve the problem.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["copy_item_detail"] = $data;
			...
			$this->log_error('Backend : Book copy', 'Command not found.', $command);
			redirect('admin/product-'.$product_main_url.'/book/edit/'.$parent_aid.'copy/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid=""){
		...
		if ($rs){
			...
			$this->log_status('Backend : Book copy', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Book copy', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		$objResult = $this->main->load_record(false);
		if(!is_var_array($objResult)){
			$msg = set_message_error('Error occurred. Can not find this book copy.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}
		...
		if ($rs){
			...
			$this->log_status('Backend : Delete book copy', $this_obj_info.' has been deleted.', $objResult);
			$this->log_product_save($this->data["this_product_type_aid"], $parent_aid, $title, "U", "Book copy [barcode = $this_obj_title] has been deleted", $objResult, "copy-delete");
	...
	function manage_column_detail($result_list, $product_main_url="0"){
		...
		foreach($result_list as $item){
			...
			$item["name_action"] = '<a href="'.site_url('admin/product-'.$product_main_url.'/book/edit/'.$parent_aid.'/copy/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"barcode","-").'</a>';
			
			// $title = get_array_value($parent_detail,"title","");
			// //echo $title;
			// $item["copy_title_action"] = '"'.$title.' '.$copy_title.'"';
			...
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inctive\' this book." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"barcode","")).'</strong>\', \'admin/product/book-copy\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this book." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"barcode","")).'</strong>\', \'admin/product/book-copy\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}
			...
			if($product_main_field_aid <= 0 && (CONST_ALLOW_DELETE_PRODUCT != "0" || is_root_admin_or_higher())){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this book." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product/book-copy\', \'<strong>'.removeAllQuote(get_array_value($item,"barcode","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
	...
	function status($product_main_url="", $parent_aid="", $type="")	{
		switch($type)
		{
			...
			case md5('no-command') : 
				...
				$this->data["js_code"] = '';
				break;
			case md5('no-book-copy') : 
				$this->data["message"] = set_message_error('Error occurred. Can not find this book copy.');
				$this->data["js_code"] = '';
				break;
			default : 
		...
		if(CONST_HAS_TRANSACTION == "1"){
			...
			if(is_var_array($user_section_result)){
				foreach ($user_section_result as $user_section) {
					...
					$this->load->model($this->book_ref_user_section_model,"book_ref_user_section");
					$this->book_ref_user_section->insert_or_update($obj);
				}
			}
		}
	}
