...
class Product_main_back_controller extends Product_init_controller {
	...
	function __construct(){
		...
		define("thisAdminSubMenu",'product_main');
		@define("folderName",'product/product_back/main');
		
		define("TXT_TITLE",'Main product management');
		define("TXT_INSERT_TITLE",'Main product management : Add new main product');
		define("TXT_UPDATE_TITLE",'Main product management : Edit main product');
				
		$this->main_model = 'Product_main_model';
...
//        $this->data["master_product_main_menu"] = $this->Product_main_model->load_product_mains();
	...
	function add(){
		...
		$this->session->set_userdata('productMainBackDataSearchSession','');
	...
	function edit($aid=""){
		...
		if(is_var_array($item_detail)){
			...
			$this->data["message"] = set_message_error('Cannot find the specific product main.');
			$this->data["js_code"] = "$('#name').focus();";
			$this->show();
			return "";
		}
	...
	function save(){
		...
		if($command == "_insert"){
			...
			if($aid > 0){
				$data["aid"] = $aid;
				$this->log_status('Backend : Insert product main', '['.$name.'] just added into database.', $data);
				redirect('admin/product-main/status/'.md5('success'));
			}else{
				$this->log_error('Backend : Insert product main', 'Command insert_record() fail. Can not insert '.$name, $data);
			...
			if($rs){
				...
				if ($aid > 0) $this->log_status('Backend : Update product main',  '['.$name.'] has been updated.', $data);
				if($save_option){
					redirect('admin/product-main/add');
				}else{
					redirect('admin/product-main/status/'.md5('success'));
				}
				return "";
			}else{
				$this->log_error('Backend : Update product main', 'Command update_record() fail. Can not update '.$name.'['.$aid.']', $data);
				$this->data["message"] = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
				$this->data["js_code"] = '';
				$this->data["command"] = $command;
				$this->data["item_detail"] = $data;
			...
			$this->log_error('Backend : Product main', 'Command not found.', $data);
			redirect('admin/product-main/status/'.md5('no-command'));
			return "";
		}
	}
	...
	function ajax_set_value($sid="", $status=""){
		...
		if ($rs){
			$this->log_status('Backend : Product main', 'Set value : "'.$f_name.'" has been set to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
			$msg = set_message_success('Data has been saved.');
			$result_obj = array("status" => 'success',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
		}else{
			$this->log_error('Backend : Product main', 'Set value : Function main->set_value() fail. Can not set "'.$f_name.'" to "'.$f_value.'" for '.$this_obj_info.'.', $objResult);
	...
	function ajax_delete_one(){
		...
		if ($rs){
			$this->log_status('Backend : Delete product main', $this_obj_info.' has been deleted.', $objResult);
			...
			$this->log_error('Backend : Delete product main', 'Command delete_records() fail. Can not delete '.$this_obj_info.'.', $objResult);
			$msg = set_message_error('Sorry, the system can not save data now. Please try again or contact your administrator.');
			$result_obj = array("status" => 'error',"msg" => $msg );
			echo json_encode($result_obj);
			return "";
	...
	function ajax_get_main_list($sid){
		...
		$dsSession = $dataSearchSession->userdata('productMainBackDataSearchSession');		
		// print_r($dsSession);
		...
		$search_product_type = $this->getDataFromInput('search_product_type');
		$data_where = "";
		if(is_var_array($search_product_type))
		foreach($search_product_type as $item){
			$data_where["product_type_aid"][] = $item;
			$data_search["search_product_type"][] = $item;
		}
		$this->main->set_where_in($data_where);
		...
		$order_by_option = $this->get_order_by_info($search_order_by,'type.weight asc, weight asc');
		...
		$header_list[] = array("sort_able"=>'1',"title_show"=>'Product type',"field_order"=>'product_type_aid',"col_show"=>'product_type_name',"title_class"=>'w200 hcenter',"result_class"=>'hcenter');
		...
		$this->session->set_userdata('productMainBackDataSearchSession',$data_search);	
	...
	function manage_column_detail($result_list){
		...
		foreach($result_list as $item){
			if(is_root_admin_or_higher()){
				$item["name_action"] = '<a href="'.site_url('admin/product-main/edit/'.get_array_value($item,"aid","")).'">'.get_array_value($item,"name","-").'</a>';
			}else{
				$item["name_action"] = ''.get_array_value($item,"name","-").'';
			}
			$status = get_array_value($item,"status","0");
			if($status == 1){
				$item["status_action"] = '<a class="btn btn-success btn-xs" title="Click to \'Inactive\' this." onclick="processChangeValue(\' inactive <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/product-main\', \''.get_array_value($item,"aid","").'\', \'status=0\')"><i class="fa fa-check"></i></a>';
			}else{
				$item["status_action"] = '<a class="btn btn-danger btn-xs" title="Click to \'Active\' this." onclick="processChangeValue(\' active <strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\', \'admin/product-main\', \''.get_array_value($item,"aid","").'\', \'status=1\')"><i class="fa fa-ban"></i></a>';
			}

			$item["action"] = '';
			$item["action"] .= '<a class="btn btn-info btn-xs" title="Click to see detail" onclick="processRedirect(\'admin/product-main-field/'.get_array_value($item,"aid","").'\')"><i class="fa fa-sitemap"></i></a>&nbsp;';
			if(is_root_admin_or_higher()){
				$item["action"] .= '<a class="btn btn-danger btn-xs" title="Click to \'Delete\' this." onclick="processDelete(\''.get_array_value($item,"aid","").'\', \'admin/product-main\', \'<strong>'.removeAllQuote(get_array_value($item,"name","")).'</strong>\')"><i class="fa fa-trash-o "></i></a>&nbsp;&nbsp;&nbsp;';
			}
			$result[] = $item;
		}
	...
	function ajax_get_product_main($sid,$product_main_aid){
		$this->load->model($this->main_model,'main');
		$this->main->set_where('aid', $product_main_aid);
		if(!is_root_admin_or_higher()) $this->main->set_where(array("status"=>"1"));
