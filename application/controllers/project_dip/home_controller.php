...
class Home_controller extends Project_init_controller {
	...
	function home()
	{
		...
		$this->data["page_title"] = "New Releases";
		...
		$product_main_list = array('1','4','3','6','7','8','5');
		...
		$this->load->model($this->view_most_comments_model,"most_comments");
		$tmp_calendar_list = $this->event->load_widget_calendar('', '1');
		$tmp_event_list = $this->event->load_home_my_invitations('', '1');
		$this->data['calendar_list'] = $tmp_calendar_list['results'];
		$this->data['event_list'] = $tmp_event_list['results'];
		$this->data['news_popular_list'] = $this->news->load_popular('1');
		$this->data['news_recommended_list'] = $this->news->load_recommended('1');
		$this->data['news_talk_of_the_town_list'] = $this->news->load_talk_of_the_town('1');
		$arr_incoming = array();
		if (is_var_array($tmp_calendar_list['results'])) {
			foreach ($tmp_calendar_list['results'] as $k => $item) {
				$format = '%d/%m/%y %H:%i น. ';
				if ($item['is_all_day'] == '1') $format = '%d/%m/%y';

				$arr_incoming[] = '<div class="inline-block" title="'.$item['title'].'"><span class="txt-orange">'.mdate($format, strtotime($item['event_start_date'])) . '</span> ' .
						(mb_strlen($item['title']) > 30 ? mb_substr($item['title'], 0, 30).'...' : $item['title'] ) .
						'</div>';
		...
		if (is_var_array($tmp_calendar_list['results'])) {
			foreach ($tmp_calendar_list['results'] as $k => $item) {
				$tmp_range = convert_date_range_to_array( $item['event_start_date'], $item['event_end_date'] );
				$arr_range = array_merge($arr_range, $tmp_range);

				foreach ($tmp_range as $d) {
					if (!isset($arr_cal[$d])) {
						$arr_cal[$d] = array();
						$arr_cal[$d]['number'] = 0;
						$arr_cal[$d]['url'] = site_url('event/detail/'.$item['aid']);
						
						if ($item['is_all_day'] == '1')
							$arr_cal[$d]['title'] = $item['title'];
						else
							$arr_cal[$d]['title'] = mdate('%H:%i น.', strtotime($item['event_start_date'])).' '.$item['title'];
					}
					else {
						if ($item['is_all_day'] == '1')
							$arr_cal[$d]['title'] .= '<br>'.$item['title'];
						else
							$arr_cal[$d]['title'] .= '<br>'.mdate('%H:%i น.', strtotime($item['event_start_date'])).' '.$item['title'];
					}
				}
			}
			foreach ($arr_range as $d) {
				if (isset($arr_cal[$d])) $arr_cal[$d]['number']++;
			}
		}
		$this->data['json_arr_cal'] = json_encode($arr_cal);
		$this->data['json_incoming_event_rotator'] = json_encode($arr_incoming);
		$tmp_rs = $this->news->load_home('', '1' , 0, 3);
		...
		// echo '<pre>';
		// print_r($arr_incoming);
		// print_r($arr_range);
		// print_r($tmp_event_list['results']);
		// print_r($arr_cal);
		// echo '</pre>';
		// exit;
