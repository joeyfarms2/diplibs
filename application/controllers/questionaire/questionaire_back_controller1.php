		...
		if ($questionaire_aid > 0) {
			...
			if(is_var_array($result_list)){
				...
				foreach ($result_list as $key => $item) {
					if (!isset($arr_questions[$item['question_aid']])) {
						$arr_questions[$item['question_aid']] = array();
						$arr_questions[$item['question_aid']]['options'] = array();
					}
					if (!isset($arr_questions[$item['question_aid']]['options'][$item['answer']])) {
						$arr_questions[$item['question_aid']]['options'][$item['answer']] = 1;
					}
					else {
						$arr_questions[$item['question_aid']]['options'][$item['answer']] = $arr_questions[$item['question_aid']]['options'][$item['answer']] + 1;
					}
					$arr_questions[$item['question_aid']]['questionaire_aid'] = $item['questionaire_aid'];
					$arr_questions[$item['question_aid']]['question_title'] = $item['question_title'];
					$arr_questions[$item['question_aid']]['question_type'] = $item['question_type'];
					$arr_questions[$item['question_aid']]['weight'] = $item['weight'];

					if (!isset($arr_result[$item['created_by']])) {
						$arr_result[$item['created_by']] = array();
						$arr_result[$item['created_by']]['full_name'] = $item['full_name'];
					}
					$arr_result[$item['created_by']][$item['question_aid']] = $item['answer'];
					$arr_result[$item['created_by']]['created_date'] = mdate('%Y-%m-%d', strtotime($item['created_date']));;
				}
				...
				$objPHPExcel->getProperties()->setTitle("Questionnaire");
				$objPHPExcel->getProperties()->setDescription("Questionnaire");
				// Set Default Style
				...
				$irow = 1;
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $irow, "#");
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $irow, "User");
				$icol = 2;
				$q = 1;
				foreach ($arr_questions as $q_aid => $item_q) {
					$toColumnLetter = PHPExcel_Cell::stringFromColumnIndex($icol); 
					$objPHPExcel->getActiveSheet()->getColumnDimension($toColumnLetter)->setWidth(50);
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($icol, $irow, "Q ".$q.": ".addslashes($item_q['question_title']));
					$icol++;
					$q++;
				}
				$toColumnLetter = PHPExcel_Cell::stringFromColumnIndex($icol); 
				$objPHPExcel->getActiveSheet()->getColumnDimension($toColumnLetter)->setWidth(20);
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($icol, $irow, "Answered Date");
				
				$sharedStyle1 = new PHPExcel_Style();
				$sharedStyle1->applyFromArray($array_style_summary_title);
				$toColumnLetter = PHPExcel_Cell::stringFromColumnIndex($icol); 
				$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A1:".$toColumnLetter."1");
				
				$irow = 2;
				$staff = 1;
				foreach($arr_result as $item_staff){
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $irow, $staff);
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $irow, get_array_value($item_staff,"full_name",""));
					$icol = 2;
					foreach ($arr_questions as $q_aid => $item_q) {
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($icol, $irow, get_array_value($item_staff, $q_aid,""));
						$icol++;
					}
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($icol, $irow, get_array_value($item_staff,"created_date",""));
					$irow++;
					$staff++;
				}
				...
				// Total row
				$irow++;
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $irow, "TOTAL");
				$icol = 2;
				foreach ($arr_questions as $q_aid => $item_q) {
					$toColumnLetter = PHPExcel_Cell::stringFromColumnIndex($icol); 
					$objPHPExcel->getActiveSheet()->getColumnDimension($toColumnLetter)->setWidth(50);
					if (is_var_array($item_q['options'])) {
						arsort($item_q['options']);
						$txt_total = "";
						foreach ($item_q['options'] as $key => $value) {
							$txt_total .= addslashes($key)." = ".addslashes($value)."\n";
						}
					}
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($icol, $irow, $txt_total);
					$icol++;
				}

				$array_style_summary_title['font']['bold'] = false;
				$array_style_summary_title['alignment']['horizontal'] = PHPExcel_Style_Alignment::HORIZONTAL_LEFT;
				$sharedStyle2 = new PHPExcel_Style();
				$sharedStyle2->applyFromArray($array_style_summary_title);
				$toColumnLetter = PHPExcel_Cell::stringFromColumnIndex($icol); 
				$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle2, "A".$irow.":".$toColumnLetter.$irow);
