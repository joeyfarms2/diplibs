...
class Event_model extends Initmodel {
	...
	function __construct()
	{
        ...
        parent::__construct();
			...
			$this->tbl_event_comment = "event_comment";
		...
		$sql .= "
			...
			UNION
				(
					...
					FROM `event` 
					LEFT JOIN `event_main` AS event_main ON `event`.`event_main_aid` = `event_main`.`aid` 
					WHERE event.is_public = '1' AND event.status = '1' AND event.event_main_aid = '1' AND DATE(event.event_start_date) >= DATE(NOW())
				)
			) AS e
			GROUP BY aid
			ORDER BY `event_start_date` ASC, `weight` ASC, `created_date` DESC
	...
	function load_widget_calendar($category_aid_arr="", $event_main_aid="", $user_aid=""){
		...
		$sql .= " OR ( MONTH(event.event_start_date) < MONTH(NOW()) ";
		$sql .= " AND YEAR(event.event_start_date) > YEAR(NOW()) )) ";
	...
	function fetch_data_with_desc($query)
	{
		...
		if($query->num_rows() > 0){
			...
			foreach($query->result_array() as $row){
				...
				$row['txt_total_join'] = ($row['total_join'] > 0 ? $row['total_join'].'<a href="javascript:void(0);" data-original-title="" class="txt-green who-join">'.($row['total_join'] > 1 ? ' people</a> join' : ' person</a> joins').'</a> this event' : 'No one joins this event.');
	...
	function update_total_comment($aid){
		if ($aid > 0) {
			$_sql = 'UPDATE '.$this->get_table_name().' 
				SET total_comment =  (SELECT count(aid) FROM '.$this->tbl_event_comment.' WHERE parent_event_aid = "'.$aid.'" and status = "1")
				WHERE aid = "'.$aid.'"';
			return $this->db->query($_sql);
		}
		return 0;
