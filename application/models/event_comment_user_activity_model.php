<?php
class Event_comment_user_activity_model extends Initmodel {
	
	function __construct()
	{
		// Call the Model constructor
        parent::__construct();
			$this->set_table_name("event_comment_user_activity");
	...
	function do_wow($event_comment_aid, $user_aid=""){
		$done = $this->insert_or_update(array(
				'event_comment_aid' => $event_comment_aid,
	...
	function do_unwow($event_comment_aid, $user_aid=""){
		$done = $this->insert_or_update(array(
				'event_comment_aid' => $event_comment_aid,
				'user_aid' => (!is_blank($user_aid) ? $user_aid : getUserLoginAid($this->user_login_info)),
				'status_wow' => '0'
	...
	function has_wowed($event_comment_aid, $user_aid=""){
		...
		if (is_var_array($result) && isset($result['event_comment_aid'])) {
			return true;
		}
		else {
			return false;
