...
class Questionaire_model extends Initmodel {
	...
	function update_total_submit($aid){
		if ($aid > 0) {
			// $_sql = 'SELECT count(user_aid) FROM '.$this->tbl_questionaire_user_submit.' WHERE questionaire_aid = "'.$aid.'" GROUP BY user_aid';
			// $tmp_rs = $this->db->query($_sql);

			$this->db->select('user_aid');
			$this->db->from($this->tbl_questionaire_user_submit);
			$this->db->where('questionaire_aid', $aid);
			$this->set_group_by("user_aid");
			$query = $this->db->get();
			$tmp_rs = $this->fetch_data($query);

			$total_submit = 0;
			if (is_var_array($tmp_rs) && !empty($tmp_rs)) {
				$total_submit = count($tmp_rs);
			}

			$_sql = 'UPDATE '.$this->get_table_name().' 
				SET total_submit =  '.$total_submit.'
				WHERE aid = "'.$aid.'"';
			return $this->db->query($_sql);
		}
		return 0;
